...
class Owa(AppBase):
    ...
    def send_email(
        ...
        m = Message(
            ...
            cc_recipients=[
                Mailbox(email_address=address)
                for address in ccrecipient.split(", ")
            ],
        )
    ...
    def mark_email_as_read(
        ...
        try:
            ...
            return {
                "success": False,
                "reason": "Email {} does not exists".format(email_id),
            }
    ...
    def move_email(
        ...
        folder = self.parse_folder(account, foldername)
        if folder["error"]:
            return {"success": False, "reason": folder["error"]}
        folder = folder["folder"]
    ...
    def get_emails(
        ...
        if auth["error"]:
            return json.dumps({"success": False, "reason": auth["error"]})
        ...
        folder = self.parse_folder(account, foldername)
        if folder["error"]:
            return json.dumps({"success": False, "reason": folder["error"]})
        ...
        include_raw_body = (
            True if include_raw_body.lower().strip() == "true" else False
        )
        include_attachment_data = (
        ...
        upload_attachments_shuffle = (
            True
            if upload_attachments_shuffle.lower().strip() == "true"
            else False
        )
        ...
        try:
            ...
            if category:
                folder_filter = folder.filter(
                    is_read=not unread, categories__icontains=category
                ).order_by("-datetime_received")[:amount]
            else:
            ...
            for email in folder_filter:
                ...
                output_dict["message_id"] = (
                    output_dict["message_id"].replace("\t", "").strip()
                )
                ...
                if upload_email_shuffle:
                    email_up = [
                        {"filename": "email.eml", "data": email.mime_content}
                    ]
                    ...
                    for attachment in email.attachments:
                        if type(attachment) == FileAttachment:
                            ...
                            atts_up.append(
                                {
                                    "filename": attachment.name,
                                    "data": attachment.item.mime_content,
                                }
                            )
                        else:
                            continue
                ...
                try:
                    if len(output_dict["body"]) > 1:
                        output_dict["body"][0]["raw_body"] = output_dict[
                            "body"
                        ][1]["content"]
